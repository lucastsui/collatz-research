...
│   │     │
│   │     PROVED RESULTS (Session 8):
│   │     ├─ THEOREM: D ∤ (3^k - 2^k) for all D = 2^p - 3^k ≥ 2.    ✅ proved
│   │     │  Proof: D odd + size (0 < 2^{p-k}-1 < D). Elementary.
│   │     │  Consequence: no cycle has consecutive odd-step positions.
│   │     │  File: theorem_consecutive_positions.md
│   │     ├─ THEOREM: k ≤ 5 by modular cascade + Steiner + parity.    ✅ proved
│   │     │  k ≤ 4: zero candidates after Steiner + v_3 filter.
│   │     │  k = 5: cascade pins (a_3,a_2,a_1)=(4,6,8), mod 4 kills residual.
│   │     │  File: k5_analysis.md
│   │     ├─ IDENTITY: w_t - 4·2^A = -3^{t+1}·n (Alpha Identity).    ✅ proved
│   │     │  Follows from Cascade-Syracuse identity by telescoping.
│   │     │  Corollary (convergence → no cycle) is trivially true without it.
│   │     │  File: theorem_alpha_identity.md
│   │     ├─ DATA: Monotone assignment never hits 0 mod D.              ~ empirical
│   │     │  Tested all (p,k) with p ≤ 500. Non-monotone hits at rate 1/D.
│   │     │  Not a theorem. Three constraints identified (powers of 2,
│   │     │  monotone order, gap ≥ 1). Removing any one yields counterexamples.
│   │     │
│   │     │  SHARP OPEN PROBLEM: For all valid (p,k) and all strictly
│   │     │  decreasing a_0 > ... > a_{k-1} ≥ 0 with a_0 < p:
│   │     │  (2^p - 3^k) ∤ Σ 2^{a_j} · 3^j.
│   │     │  This is equivalent to Collatz Part 1.
│   │     │  Consecutive case proved (Theorem above). General case open.
│   │     │
│   │     No known framework for the general case.
...
| `theorem_consecutive_positions.md` | **Session 8: THEOREM — D ∤ (3^k - 2^k), consecutive positions cannot cycle** |
| `theorem_alpha_identity.md` | **Session 8: Alpha Identity w_t = 4·2^A - 3^{t+1}·n (telescoping proof)** |
| `k5_analysis.md` | **Session 8: k=5 proof by modular cascade (pins exponents, mod 4 obstruction)** |
| `solve_k5.py` | **Session 8: Computational verification for k=5 equation** |
| `paper_alpha_identity.md` | **Session 8: Clean writeup of all results with honest assessment** |
| `SHARING_GUIDE.md` | **Session 8: Researcher contact list for sharing results** |
